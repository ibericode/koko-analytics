...
class Plausible_Importer extends Importer
{
    ...
    private function import_page_stats($fh, array $headers, string $date_start, string $date_end): void
    {
        ...
        while ($row = fgetcsv($fh, 1024, ',', '"', '')) {
            ...
            if (count($rows) >= 100) {
                $this->bulk_insert_page_stats($rows);
                $rows = [];
            }
        }
    ...
    private function import_referrer_stats($fh, array $headers, string $date_start, string $date_end): void
    {
        ...
        while ($row = fgetcsv($fh, 1024, ',', '"', '')) {
            ...
            if (count($rows) >= 100) {
                $this->bulk_insert_referrer_stats($rows);
                $rows = [];
            }
        }
