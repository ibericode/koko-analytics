...
$date_range = (new \KokoAnalytics\Import\WP_Statistics_Importer())->get_available_date_range();
...
<h1 class="mt-0"><?php esc_html_e('Import from WP Statistics', 'koko-analytics'); ?></h1>
<p><?php esc_html_e('Import your historical site, page and referrer statistics from WP Statistics into Koko Analytics.', 'koko-analytics'); ?></p>

<?php if ($date_range === null) { ?>
    <div class="ka-alert ka-alert-warning" role="alert">
        <?php esc_html_e('No WP Statistics data was found.', 'koko-analytics'); ?>
    </div>
<?php } else { ?>
    <form method="post" onsubmit="return confirm('<?php esc_attr_e('Are you sure you want to import statistics between', 'koko-analytics'); ?> ' + this['date-start'].value + '<?php esc_attr_e(' and ', 'koko-analytics'); ?>' + this['date-end'].value + '<?php esc_attr_e('? This will add to any existing data in your Koko Analytics database tables.', 'koko-analytics'); ?>');" action="">
        <input type="hidden" name="koko_analytics_action" value="start_wp_statistics_import">
        <?php wp_nonce_field('koko_analytics_start_wp_statistics_import'); ?>
