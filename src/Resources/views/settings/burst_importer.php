...
$date_range = (new \KokoAnalytics\Import\Burst_Importer())->get_available_date_range();
...
<h1 class="mt-0"><?php esc_html_e('Import from Burst Statistics', 'koko-analytics'); ?></h1>
<p><?php esc_html_e('Import your historical analytics data from Burst Statistics into Koko Analytics.', 'koko-analytics'); ?></p>

<?php if ($date_range === null) { ?>
    <div class="ka-alert ka-alert-warning" role="alert">
        <?php esc_html_e('No Burst Statistics data was found.', 'koko-analytics'); ?>
    </div>
<?php } else { ?>
