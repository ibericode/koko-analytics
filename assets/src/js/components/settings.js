...
export default function Settings({history}) {
  let [settings, setSettings] = useState(window.koko_analytics.settings)
  let [saving, setSaving] = useState(false)
  let [buttonText, setButtonText] = useState(__('Save Changes', 'koko-analytics'))

  function onSubmit(evt) {
    evt.preventDefault()
    setSaving(true)
    setButtonText(__('Saving - please wait', 'koko-analytics'))

    request('/settings', {
      method: 'POST',
      body: settings
    }).then(() => {
      setButtonText(__('Saved!', 'koko-analytics'))
    }).finally(() => {
      // re-enable button after 4 seconds
      setTimeout(() => {
        setSaving(false)
        setButtonText(__('Save Changes', 'koko-analytics'))
      }, 4000)
    })
  }

  function handleRadioClick (key) {
    return (evt) => {
          ...
          <form method='POST' onSubmit={onSubmit}>
            <div className='input-group'>
              ...
              <select
                ...
                setSettings({...settings, exclude_user_roles: [].filter.call(evt.target.options, el => el.selected).map(el => el.value) })
              }}
              >
            ...
            <div className='input-group'>
              ...
              <label className='cb'><input type='radio' name='use_cookie' value={1} onChange={handleRadioClick('use_cookie')} checked={settings.use_cookie === 1} /> {__('Yes', 'koko-analytics')}</label>
              <label className='cb'><input type='radio' name='use_cookie' value={0} onChange={handleRadioClick('use_cookie')} checked={settings.use_cookie === 0} /> {__('No', 'koko-analytics')}</label>
            ...
            <div className='input-group'>
              ...
              <select
                ...
                setSettings({...settings, default_view: evt.target.value})
              }}
            ...
            <div className='input-group'>
              ...
              <input
                ...
                setSettings({...settings, prune_data_after_months: parseInt(evt.target.value)})
